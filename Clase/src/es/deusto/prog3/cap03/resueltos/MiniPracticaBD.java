...
public class MiniPracticaBD {
				...
				if (!tfUsuario.getText().isEmpty() && !tfPassword.getText().isEmpty()) {
					...
					try {
						...
						if (numFilasCambiadas == 0) {
							JOptionPane.showMessageDialog( ventana, "No existía un usuario con ese nick " + tfUsuario.getText() );
						}
					} catch (SQLException e2) {
						System.out.println( "Último comando: " + com );
						e2.printStackTrace();
	...
	// Posible ataque sin este método:  o'; drop table Usuario;    en el cuadro de texto de nick - botón borrar
